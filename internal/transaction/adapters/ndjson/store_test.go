...
func ndjsonAcceptance() app.Acceptance {
	...
	return app.Acceptance{Transaction: transaction, Fingerprint: fingerprint, Event: app.OutboxEvent{
		...
		OccurredAt: time.Date(2026, 8, 16, 22, 0, 0, 0, time.UTC), Source: transaction.Source, SourceReference: transaction.SourceReference,
...
func TestStorePersistsRawAuditBytesAcrossRestart(t *testing.T) {
	path := filepath.Join(t.TempDir(), "transactions.ndjson")
	acceptance := ndjsonAcceptance()
	acceptance.Transaction.MetadataRaw = json.RawMessage(`{ "large": 9007199254740993, "rate": 12.50 }`)
	acceptance.Transaction.LocationRaw = json.RawMessage(`{ "lane" : "3" }`)
	...
	claimed, err := reopened.ClaimPending(context.Background(), time.Now(), time.Minute, 1)
	if err != nil || len(claimed) != 1 {
		t.Fatalf("claim=%#v err=%v", claimed, err)
	}
	if !bytes.Equal(claimed[0].Event.Payload.MetadataRaw, acceptance.Transaction.MetadataRaw) || !bytes.Equal(claimed[0].Event.Payload.LocationRaw, acceptance.Transaction.LocationRaw) {
		t.Fatalf("raw bytes changed across restart: %#v", claimed[0].Event.Payload)
	}
}
