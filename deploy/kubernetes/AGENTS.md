...
- Load application credentials directly from AWS Secrets Manager through IRSA; reserve Kubernetes Secret references for the bootstrap Job until it uses the same provider.
...
| `topic-bootstrap-job.yaml` | Runs the idempotent topic configuration after MSK readiness. |
