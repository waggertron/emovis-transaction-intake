...
umask 077
printf '%s' '{"API_KEY":"local-development-only-key","PARTNER_ID":"local-partner","AUTH_MODE":"api_key","TRANSACTION_TYPES":"toll","DEFAULT_CURRENCY":"USD","HTTP_ADDRESS":":8080","KAFKA_BROKERS":"kafka:9092","KAFKA_TOPIC":"transaction.review-candidates.v1","STORE_DRIVER":"ndjson","STORE_PATH":"/data/transactions.ndjson"}' >"${secret_file}"
...
printf '%s' '{"API_KEY":"rotated-local-key","PARTNER_ID":"local-partner","AUTH_MODE":"api_key","TRANSACTION_TYPES":"toll","DEFAULT_CURRENCY":"USD","HTTP_ADDRESS":":8080","KAFKA_BROKERS":"kafka:9092","KAFKA_TOPIC":"transaction.review-candidates.v1","STORE_DRIVER":"ndjson","STORE_PATH":"/data/transactions.ndjson"}' >"${secret_file}"
...
status="$(curl --silent --show-error -o "${E2E_TEMP_DIR}/old-key.json" -w '%{http_code}' \
  ...
  --data "${E2E_REQUEST}" http://127.0.0.1:18083/ingest/v1/transactions)"
