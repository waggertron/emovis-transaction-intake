...
- **User — Keep checklist items atomic and evidence-based.** Separate compound requirements into independently numbered checklist items, check off only the portions fully supported by repository or execution evidence, and leave incomplete portions visible as their own unchecked items.
- **User — Require production-shaped component testing.** Every external-service implementation must be exercised through the production adapter against its real local substitute, reuse the shared behavioral contract where applicable, have an explicit Make-backed target, test success and dependency-failure behavior, and remove all containers, volumes, networks, ports, credentials, certificates, files, and seeded state; mocks and configuration-only assertions remain unit evidence and cannot satisfy a component requirement.
- **User — Require end-to-end tests for every local implementation.** Memory, NDJSON, PostgreSQL, DynamoDB Local, local secrets, and Kafka configurations must each be exercised through the production HTTP intake, selected storage adapter, outbox worker, Kafka publish/consume boundary, observable replay/conflict behavior, and complete cleanup; component contracts do not replace these end-to-end paths.
- **User — Close with checklist confirmation and repository handoff.** The final work item confirms every checklist item is complete, then sends the repository URL.
...
- **2026-08-17 — User — Keep the handoff repository private.** Change `waggertron/emovis-transaction-intake` from public to private while retaining its URL; this supersedes the earlier public-visibility handoff requirement and requires explicit GitHub access for reviewers.
- **2026-08-17 — Codex — Make hosted validation dependencies and secret-fixture ownership explicit.** Install `ripgrep` in the infrastructure CI job, enforce that prerequisite with a workflow contract, and run the Linux secret-provider E2E service plus its capability-limited data-volume initializer as the non-root host fixture owner so the secret remains mode `0600` across Docker Desktop and GitHub Linux runners.
...
- [x] **1.14** Smoke-test a fresh repository checkout after the full instruction hierarchy is present: confirm every authored directory includes a valid `AGENTS.md`, and that plans, skills, code, deployment, and infrastructure instructions are discoverable without generated or local-only artifacts.
...
- [x] **3.1** Create the labeled mock OpenAPI 3.1 contract for `POST /v1/transactions`, `/healthz`, `/readyz`, and `/metrics`.
- [x] **3.2** Define request fields: UUID transaction ID, RFC 3339 occurred-at, positive minor-unit amount, three-letter uppercase currency, agency/plaza/lane IDs, and vehicle class.
- [x] **3.3** Define responses: `201` new acceptance; `200` and `Idempotent-Replay: true` for identical replay; `409` conflict; `400` malformed JSON; `401` invalid API key; `422` semantic invalidity.
- [x] **3.4** Implement only enough Go code to satisfy the prewritten tests: domain validation, application use cases, HTTP adapter, request IDs, structured logs, configuration, and graceful shutdown.
...
- [x] **4.1** Add Docker Compose with a single-node KRaft Kafka broker and one combined API/dispatcher service.
...
- [x] **6.3** Create and push repository `waggertron/emovis-transaction-intake`; verify hosted README and CI workflow. The repository was initially public and was changed to private by the later user decision.
...
- [x] **6.5** Smoke-test the published repository from a fresh clone: install declared dependencies, run the unit suite, start the documented local stack, submit the README example, observe the Kafka event, and perform the documented cleanup.
- [x] **6.6** Validate repository handoff quality: all links work, no secrets or generated artifacts are tracked, CI is green, Git history contains the plan and skill changes, and every checklist item has recorded evidence.
...
- [x] **6.14** Confirm every preceding plan checklist item is complete, then send the private repository URL to the authorized reviewer.
- [x] **6.15** Move this completed timestamped plan from `docs/plans/current/` to `docs/plans/archive/` as the final repository content action; update any plan links, validate the archive location, and confirm `docs/plans/current/` contains no completed plan.

## Execution Evidence

- **2026-08-17 — Fresh-clone and final hosted handoff.** A fresh clone of commit `b568eca` passed `go test ./...`, Markdown-link validation, the 58-directory instruction hierarchy, and `make smoke`; the repeated post-cleanup audit found no Emovis containers, volumes, networks, or tracked changes. After test-first Linux portability fixes, private GitHub Actions run `32012789498` passed quality, infrastructure, security, smoke, every-local-implementation E2E, and real external-service component jobs at commit `336e98d`.
- **2026-08-17 — Final checklist confirmation.** Reconciled every numbered item against tracked implementation, local red-green evidence, fresh-clone evidence, security review, coverage reports, Docker cleanup, Git history, and green private hosted CI. All preceding checklist items are complete; repository handoff is `https://github.com/waggertron/emovis-transaction-intake` for authorized reviewers.

- **2026-08-17 — Checklist reconciliation.** Reviewed all checklist wording against tracked files, local validation, and hosted CI run `32003786985`. Split mixed completed/pending requirements into atomic items, marked 54 of 124 items complete, and retained 70 items as explicitly pending; no item was checked solely because related code exists.
- **2026-08-17 — Per-package coverage gate (red).** `make coverage` rejected topic-bootstrap 46.9%, transaction-service 22.2%, DynamoDB 81.4%, HTTP 83.5%, NDJSON 76.6%, and PostgreSQL 76.2%; it also demonstrated that the prior 72.2% aggregate could not satisfy a per-package gate.
